...
console.log("\nProduction (run these from apps/api/, so wrangler picks up wrangler.toml");
console.log("and the pinned local version):");
console.log(`  echo '${publicKey}' | bunx wrangler secret put VAPID_PUBLIC_KEY --env production`);
console.log(`  echo '${privateKey}' | bunx wrangler secret put VAPID_PRIVATE_KEY --env production`);
console.log("  openssl rand -hex 32 | bunx wrangler secret put EVENTSUB_SECRET --env production");
